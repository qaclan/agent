...
- Run execution (`web/routes/runs.py`): one subprocess per script under `~/.qaclan/runs/<run_id>/`. Shared `state.json` in that dir carries cookies / localStorage between scripts — the cross-language state mechanism. Per-script timeout is 300s. `PLAYWRIGHT_BROWSERS_PATH` resolved via `runtime_setup.browsers_path_if_present()` (runtime), then frozen-binary default, then unset.
