...
- **Maintenance rule:** any change to response-schema-check behavior — `diff_schemas`/severity/verdict in `cli/schema_diff.py`, the runner wiring in `cli/api_runner.py`, resolution/capture in `web/api/services/runner_service.py`, the `schema_check`/`schema_check_default`/`schema_drift` columns and the frozen-`response_schema` baseline in `cli/db.py` and their repositories (`request_repo.py`, `collection_repo.py`, `collection_run_repo.py`, `api_run_repo.py`), sync mapping (`cli/sync.py`, `cli/commands/pull.py`), the HTML report (`cli/api_report.py`), or the UI (the shared drift renderer `web/static/api/components/schema-diff-view.js`, `web/static/api/components/response-panel.js`, `web/static/api/views/request-editor-view.js`, `collection-detail-view.js`, `collection-run-view.js`, the Runs history modal in `web/static/app.js`) — must be reflected in `docs/api-schema-check-reference.md` in the same change. Grep for `schema_check\|response_schema\|schema_drift\|diff_schemas` if unsure whether a touched file is in scope.
- Negative API testing (opt-in): from a happy-path request the tool auto-generates negative cases across three categories (input-validation, request-level mutators, injection/fuzz), runs each as a mutated re-send, and classifies a severity-based verdict where a **false pass** (API accepted invalid input) is critical. Generation lives in `cli/negative_gen.py` (enriched by OpenAPI `field_constraints` when present); severity/verdict in `cli/negative_check.py`; the mutated sends (`_apply_mutation`/`_run_negative_case`) run in `cli/api_runner.py::run_api_request`; enablement resolution (request `negative_check` tri-state + collection `negative_check_default`), `generate_negatives`, and the mutating-verb **safety gate** live in `web/api/services/runner_service.py`. Verdicts persist as `negative_result` on `api_request_results`/`api_runs`. Ordinary "Send" never fires negatives — only the editor's "Run negatives" action or a collection run does. Full reference: **`docs/api-negative-testing-reference.md`**.
- **Maintenance rule:** any change to negative-testing behavior — generation in `cli/negative_gen.py`, classification/severity in `cli/negative_check.py`, the runner wiring in `cli/api_runner.py`, resolution/`run_negatives`/`generate_negatives`/safety gate in `web/api/services/runner_service.py`, the `negative_cases`/`negative_check`/`field_constraints`/`negative_check_default`/`negative_result` columns in `cli/db.py` and their repositories (`request_repo.py`, `collection_repo.py`, `collection_run_repo.py`, `api_run_repo.py`), OpenAPI constraint extraction in `cli/api_discovery/openapi_parser.py`, routes (`web/api/routes/requests.py`, `collections.py`), the CLI (`cli/commands/api_cmd.py`), sync (`cli/sync.py`, `cli/commands/pull.py`), the HTML report (`cli/api_report.py`), or the UI (the shared renderer `web/static/api/components/negative-view.js`, `web/static/api/components/response-panel.js`, `web/static/api/views/request-editor-view.js`, `collection-detail-view.js`, `collection-run-view.js`, `collections-view.js` (request-list feature markers), the collection run chooser `web/static/api/api-section.js::qcCollectionRunConfirm`, the Runs history modal in `web/static/app.js`) — must be reflected in `docs/api-negative-testing-reference.md` in the same change. Grep for `negative_cases\|negative_check\|negative_result\|field_constraints\|negative_gen\|negatives_mode\|qcCollectionRunConfirm` if unsure whether a touched file is in scope.
