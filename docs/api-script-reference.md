...
**Persistence across runs, and environment scoping:** when a request belongs to a collection, `resolve_and_run_api_item` (`web/api/services/runner_service.py`) additionally persists each `qc.set` output into that collection's variable store (`collection_vars`) via `CollectionVarsRepo.upsert_runtime()`, tagged with the environment bound to the collection at that moment (`runtime_value`/`runtime_env_name` columns — separate from `initial_value`, the user-authored static default, which `qc.set` never touches). Before a later run, `CollectionVarsRepo.as_seed_dict(collection_id, env_name)` seeds `qaclan_vars` from this store: a variable's runtime value is used only if its tagged environment still matches the collection's currently bound environment; otherwise the seed falls back to the static `initial_value`. Practically: a `qc.set("access_token", ...)` captured while environment "dev" is bound keeps applying across runs as long as "dev" stays bound, but switching the collection to a different environment (or unbinding it) drops that captured value automatically — the request then resolves `access_token` from the newly-bound environment's own `env_vars`, or the static default, instead of the stale "dev" token. The collection-vars UI marks a variable whose runtime override is currently applying with a "live" badge, distinct from its static default.
...
- No 4-tier variable scoping (global/collection/environment/local) — one flat `qaclan_vars` bucket plus the active environment's `env_vars`. Likely intentional given the local-first/single-active-environment model rather than a bug. Both tiers support secret values (`is_secret`) — see "Variable secrecy" above. A `qc.set` value persisted to `collection_vars` does carry one dimension of environment scoping (see "What writes to `state.json`" above) — it applies only while the environment it was captured under stays bound — but this is not a general per-environment history: only the single most recent capture is kept per variable.
