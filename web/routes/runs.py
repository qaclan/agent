...
from cli.config import get_active_project_id, QACLAN_DIR, UPLOADS_DIR
...
def execute_run():
    ...
    try:
        ...
        # Wrapped in try/finally so an unexpected exception mid-loop (e.g. a
        # bug in one item's handling) still finalizes suite_runs instead of
        # leaving it stuck at RUNNING forever — see commit 42a508f which
        # applied the same guard to the sibling API-collection run loop.
        try:
            for idx, item in enumerate(items):
                ...
                if stopped:
                    logger.info("execute_run: [%d/%d] %s — SKIPPED (stop-on-fail)", idx + 1, total, item["script_name"] or item["api_request_id"])
                    if item["item_type"] == "api_request":
                        skipped += 1
                        script_results.append({
                            "item_type": "api_request",
                            "api_request_id": item["api_request_id"],
                            "name": item["api_request_id"],
                            "status": "SKIPPED",
                            "duration_ms": 0,
                            "error_message": None,
                        })
                        ...
                        script_results.append({
                            ...
                            "name": item["script_name"] or item["api_request_id"],
                            "status": "SKIPPED",
                            "duration_ms": 0,
                            "error_message": None,
                        })
